...
resource "azurerm_key_vault" "kv_user" {
  ...
  network_acls {
    ...
    virtual_network_subnet_ids = var.use_private_endpoint ? [local.management_subnet_exists ? data.azurerm_subnet.subnet_management[0].id : azurerm_subnet.subnet_management[0].id] : []
  }
